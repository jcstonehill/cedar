...
class AdiabaticTransient0D(cedar.Benchmark):
    ...
    def compute(cls) -> dict:
        mesh = cedar.Mesh3D(
            ...
            + "/data/adiabatic_transient_0d.vtkhdf"
        )
        ...
        ht.T.ic = 0
        ht.add_source("volume", cedar.HeatTransferVolumetricSource(1))
        ...
        for i in range(25):
            ...
            T_vals[i + 1] = np.average(ht.T.get())
        ...
        return (
            {"T": T_vals},
            {"T": np.linspace(0.0, 25.0, 26)},
            {"t_array": np.linspace(0.0, 25.0, 26)},
        )
