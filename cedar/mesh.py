...
class Mesh(ABC):
    ...
    def get_local_cell_i(self, global_cell_i: int) -> tuple[str, int]:
        ...
        raise Exception(
            "Can't find this global_cell_i in any region: " + str(global_cell_i)
        )
...
class Mesh1D(Mesh):
    ...
    def _build(self):
        ...
        self.cell_pts_i: np.ndarray = np.array(
            [(i, i + 1) for i in range(self.N_cells)]
        )
        self.cell_faces_i: np.ndarray = np.copy(
            self.cell_pts_i
        ...
        self.face_centers: np.ndarray = np.copy(
            self.pts
        )  # For 1D, points and faces are the same thing

        self.face_n: np.ndarray = np.full(
            (self.N_faces, 3), self.basis_vec[self.basis], dtype=np.float64
        )
        self.face_d: np.ndarray = np.full(
            (self.N_faces, 2), self.ds / 2, dtype=np.float64
        )
        self.face_d[0][1] = 0
        self.face_d[-1][1] = 0
        ...
        self.region_i: dict[str, np.ndarray] = {
            self.regions[0]: np.arange(self.N_cells)
        }
...
class Mesh3D(Mesh):
    def __init__(self, file: str):
        ...
        self.boundaries: list[str] = self._get_boundaries()
    ...
    def _get_boundaries(self) -> list[str]:
        boundaries: list[str] = []
    ...
    def _calc_face_data(self):
        ...
        self.face_n = (cross_prod / (cross_norms[:, np.newaxis] + 1e-12)).astype(
            np.float64
        )
        ...
        for boundary in self.boundaries:
            self.boundary_area[boundary] = np.sum(
                self.face_areas[self.boundary_i[boundary]]
            )
    ...
    def _vtkhdf_dict(self):
        ...
        for region in self.regions:
            vtkhdf_dict["blocks"][region] = {
                ...
                "Types": (
                    np.full(self.region_N[region], 10),
                    "uint8",
                ),  # 10 means VTK_TETRA
                "NumberOfConnectivityIds": ((4 * self.region_N[region],), "i8"),
                "Connectivity": (
                    np.ravel(self.cell_pts_i[self.region_i[region]]),
                    "i8",
                ),
        ...
        for boundary in self.boundaries:
            vtkhdf_dict["blocks"][boundary] = {
                ...
                "Types": (
                    np.full(self.boundary_N[boundary], 5),
                    "uint8",
                ),  # 5 means VTK_TRIANGLE
                "NumberOfConnectivityIds": ((3 * self.boundary_N[boundary],), "i8"),
                "Connectivity": (
                    np.ravel(self.face_pts_i[self.boundary_i[boundary]]),
                    "i8",
                ),
                "Offsets": (
                    np.arange((self.boundary_N[boundary] + 1) * 3, step=3),
                    "i8",
                ),
            }
